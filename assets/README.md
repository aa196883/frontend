...
- `client/` – created temporarily by [`install_client.sh`](../install_client.sh).
  The script clones the private Vue.js client repository from GitLab, installs
...
- `data/` – created by [`install_data.sh`](../install_data.sh). The script clones
  the private musical dataset repository from GitLab and uses its `Makefile` to
  generate the alternate formats consumed by the frontend.

Both GitLab repositories require authentication. If you do not have permission
(or if network access is blocked) the scripts will fail during the `git clone`
step, leaving the directories absent.
